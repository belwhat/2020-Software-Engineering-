...
提交至文件夹2020-11-18-tasks

01 核心算法：要添加使用这种算法的依据（与其他算法的对比，为什么好  可以做一个摘要）  吴杰乐

02 格式：图1-1 xxx用例图.... 表1-1xxx功能 文中写xxx参考图1-1xxx用例图要规范  潘雨非

03 数据爬取流程：来源和调研会放在需求文档里，so，你的任务是给设计文档写出 爬虫语言，运行环境环境  公网ip  如何爬取，存放和数据预处理（如文件格式处理.xls .xlsx等，数据格式处理 要知道保留哪些信息）  注意设计定期更新和与原来数据作比较 如何防止被屏蔽（如数据代理）  李斯文
